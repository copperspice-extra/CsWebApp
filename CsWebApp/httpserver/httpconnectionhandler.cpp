...
HttpConnectionHandler::HttpConnectionHandler(const QSettings *settings,
      HttpRequestHandler *requestHandler, const QSslConfiguration* sslConfiguration)
   : QObject()
{
...
void HttpConnectionHandler::handleConnection(tSocketDescriptor socketDescriptor)
{
    ...
    // workaround - we need to clear writebuffer before reusing this socket
    socket->connectToHost("",0);
    socket->abort();

    if (!socket->setSocketDescriptor(socketDescriptor))
    {
        qCritical("HttpConnectionHandler (%p): Can not initialize socket: %s",
                  static_cast<void*>(this), csPrintable(socket->errorString()));
        return;
    }
...
void HttpConnectionHandler::read()
{
   ...
   while (socket->bytesAvailable() &&
      currentRequest->getStatus()!=HttpRequest::complete && currentRequest->getStatus() != HttpRequest::abort) {
   ...
   if (currentRequest->getStatus()==HttpRequest::complete) {
      ...
      if (! closeConnection) {
         ...
         if (closeResponse == true) {
            ...
            if (! hasContentLength) {
               bool hasChunkedMode=QString::compare(response.getHeaders()
                     .value("Transfer-Encoding"),"chunked",Qt::CaseInsensitive)==0;
