...
void HttpRequest::readRequest(QTcpSocket* socket)
{
   ...
   if (! newData.isEmpty()) {
      qDebug("HttpRequest: from %s: %s", csPrintable(socket->peerAddress().toString()),newData.data());
...
void HttpRequest::parseMultiPartFile()
{
         ...
         if (line.startsWith("--"+boundary)) {
            ...
            if (fileName.isEmpty() && !fieldName.isEmpty()) {
               ...
               if (uploadedFile->error()) {
                  qCritical("HttpRequest: error writing temp file, %s", csPrintable(uploadedFile->errorString()));
               }
            }
         }
      }
   ...
   if (tempFile->error()) {
      qCritical("HttpRequest: Can not read temp file, %s", csPrintable(tempFile->errorString()));
   }
