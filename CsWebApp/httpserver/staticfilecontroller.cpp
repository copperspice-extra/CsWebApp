    ...
    :HttpRequestHandler(parent)
{
   ...
   qDebug("StaticFileController: docroot=%s, encoding=%s, maxAge=%i", csPrintable(docroot), csPrintable(encoding),maxAge);
...
void StaticFileController::service(HttpRequest &request, HttpResponse &response)
{
   ...
   if (entry && (cacheTimeout==0 || entry->created>now-cacheTimeout)) {
      //copy the cached document, because other threads may destroy the cached entry immediately after mutex unlock.
      ...
      qDebug("StaticFileController: Open file %s", csPrintable(file.fileName()));
      ...
      if (file.open(QIODevice::ReadOnly)) {
         ...
         if (file.exists()) {
            qWarning("StaticFileController: Can not open existing file %s for reading", csPrintable(file.fileName()));
            response.setStatus(403,"forbidden");
            response.write("403 forbidden",true);
...
void StaticFileController::setContentType(const QString fileName, HttpResponse &response) const
{
    ...
    {
        response.setHeader("Content-Type", csPrintable("text/plain; charset="+encoding));
    }
    else if (fileName.endsWith(".html") || fileName.endsWith(".htm"))
    {
        response.setHeader("Content-Type", csPrintable("text/html; charset="+encoding));
    }
    ...
    {
        qDebug("StaticFileController: unknown MIME type for filename '%s'", csPrintable(fileName));
    }
}
